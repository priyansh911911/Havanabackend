...
router.delete('/:id', auth, authorize('ADMIN'), laundryController.deleteLaundry);

// Item Management
router.patch('/item/:itemId/status', auth, authorize(['ADMIN', 'GM', 'ACCOUNTS', 'FRONT DESK']), laundryController.updateItemStatus);
router.patch('/:id/vendor-details', auth, authorize(['STAFF','ADMIN', 'FRONT DESK', 'ADMIN']), laundryController.updateVendorDetails);
